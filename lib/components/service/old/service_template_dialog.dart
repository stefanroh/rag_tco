...
import 'package:rag_tco/components/service/old/provider_selector.dart';
import 'package:rag_tco/data_model/old/provider_information.dart';
import 'package:rag_tco/data_model/old/service_template.dart';
...
class _ServiceTemplateDialogState extends ConsumerState<ServiceTemplateDialog> {
  ...
  Future<List<ServiceTemplate>> getTemplateData(String path) async {
    ...
    List<UnitTypesLegacy> templateComponentType = [];
  ...
  String stringBuilder(ServiceTemplate template) {
    ...
    for (int i = 0; i < template.componentNames.length; i++) {
      returnString +=
          "${template.componentNames[i]}: ${template.componentAmounts[i]} ${ProviderInformation.getUnitTypesLegacytring(template.componentUnits[i])}";
